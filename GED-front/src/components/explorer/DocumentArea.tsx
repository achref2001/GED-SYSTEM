...
export function DocumentArea({ selectedTags: propTags }: { selectedTags?: string[] }) {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-20 animate-in fade-in duration-700 bg-transparent">
        <div className="w-24 h-24 rounded-[2rem] bg-indigo-500/5 flex items-center justify-center mb-8 relative">
           <div className="absolute inset-0 rounded-[2rem] border-2 border-indigo-500/10 border-t-indigo-500 animate-spin" />
           <Loader2 className="w-10 h-10 text-indigo-500 animate-pulse" />
        </div>
  ...
  if (isError) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-20 glass-card mx-20 rounded-[3rem] border-rose-100 bg-rose-50/20">
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-20">
        <motion.div 
           ...
           className="w-40 h-40 rounded-[3rem] bg-white border border-indigo-50 shadow-2xl shadow-indigo-500/5 flex items-center justify-center text-indigo-100 mb-12 relative"
        >
           <FolderSearch size={64} className="drop-shadow-sm opacity-20" />
           <motion.div 
             animate={{ y: [0, -10, 0] }}
             transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
             className="absolute -bottom-2 -right-2 w-14 h-14 rounded-2xl bg-indigo-600 flex items-center justify-center text-white shadow-2xl shadow-indigo-600/40 border-4 border-white"
           >
              <LayoutGrid size={24} />
           </motion.div>
        </motion.div>
