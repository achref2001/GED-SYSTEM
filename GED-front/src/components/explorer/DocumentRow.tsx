...
export function DocumentRow({ document: doc }: { document: Document }) {
  ...
  return (
    <div 
      className={cn(
        "group grid grid-cols-12 items-center px-6 py-4 rounded-[1.5rem] cursor-pointer transition-all duration-300 active:scale-[0.99] border-2 border-transparent",
        isSelected 
          ? "bg-blue-600 text-white shadow-2xl shadow-blue-500/30 scale-[1.01] border-blue-600 font-bold" 
          : "hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 hover:border-slate-50 text-slate-600 hover:text-slate-900",
        doc.is_locked && !isSelected && "bg-amber-50/50"
      )}
      onClick={handleClick}
    >
      {/* Identity Group */}
      <div className="col-span-6 flex items-center gap-4 min-w-0 pr-6 group/id">
         <div 
           className={cn(
      ...
      {/* Status Meta Icons */}
      <div className="col-span-2 flex justify-center gap-1.5 transition-all">
         <AnimatePresence>
            {doc.is_locked && (
      ...
      {/* Date Interaction */}
      <div className={cn("col-span-2 text-center text-[10px] font-black uppercase tracking-widest flex flex-col transition-opacity", isSelected ? "text-white" : "text-slate-500")}>
      ...
      <div className="col-span-2 flex items-center justify-end gap-6 text-[10px] font-black uppercase tracking-widest group/footer pr-4">
